...
from VXTool.core import Color, Dot
...
class DotTestCase(unittest.TestCase):
    ...
    def test_dot_hash(self):
        ...
        self.assertNotEqual(
            hash(dot), hash(new_dot), "attr different from pos didn' changed hash"
        )

    def test_variant(self):
        new_dot = DotTestCase.base_dot.variant(pos=(1, 1), letter="B", clear=False)
    ...
    def test_variant_performance(self):
        ...
        p = Timer(
            ...
            func=lambda _: dot.variant(
                pos=(2, 2), letter="A", color=Color(125, 125, 255), font_size=16
            ),
        ).run()
    ...
    def test_hash_performance(self):
        dot = DotTestCase.base_dot
        p = Timer(number=100000, func=lambda _: hash(dot)).run()
        print(self.id(), p.stats)
