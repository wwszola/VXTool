...
def plot_func(
    func, size, x_range=(0, 1), y_range=(-1, 1), offset=(0, 0), snap: Callable = round
):
    W, H = size
    left_W = -(W // 2)
    right_W = W + left_W
...
def plot_under_func(
    func, size, x_range=(0, 1), y_range=(-1, 1), offset=(0, 0), snap=round, y_end=0
):
...
def words_bound(text: str, region: Rect) -> Generator:
    ...
    while len(text) > 0 and pos[1] < region.height:
        ...
        word = text[:cut]
        text = text[cut:]
        yield from words_line(word, (pos[0] + region.left, pos[1] + region.top))
...
def polygon_seq(
    n: int, center: tuple[int, int], radius: int, offset: float = 0.0
) -> Generator:
    if n < 1:
        return None
    elif n == 1:
